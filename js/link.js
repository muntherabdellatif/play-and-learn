const types = ["Drawing" , "Typing" , "Programming" , "Electronic" , "English"] ;

const allGames = [
    {
        category : ["Drawing"],
        paid : "free" ,
        link :"https://gartic.io/" ,
        img : "./img/Gartic.png" ,
        name : "Gartic" , 
        description : "Draw and guess the words as fast as you can! Join on worldwide rooms or create your own." 
    } ,
    {
        category : ["Typing"],
        paid : "paid" ,
        link : "https://www.typingclub.com/sportal/program-3/116.play" ,
        img : "./img/typing-club.png" ,
        name : "Typing Club" , 
        description : "Learn touch typing online using TypingClubs courses. It includes 650 typing games, typing tests and videos." 
    } ,
    {
        category : ["Programming"],
        paid : "free" ,
        link : "https://flexboxfroggy.com/" ,
        img : "./img/flexboxfroggy.png" ,
        name : "Flexbox Froggy" , 
        description : " A game for learning CSS flexbox ." 
    } ,
    {
        category : ["Drawing","Programming" ,"Electronic"],
        paid : "free" ,
        link : "https://www.tinkercad.com/" ,
        img : "./img/Tinkercad.jpg" ,
        name : "Tinkercad" , 
        description : "Tinkercad is a free, easy-to-use web app that equips the next generation of designers and engineers with the foundational skills for innovation: 3D design, electronics, and coding!" 
    } ,
    {
        category : ["English"],
        paid : "free" ,
        link : "https://www.spyfall.app/" ,
        img : "./img/spyfall.png" ,
        name : "spyfall" , 
        description : "Free online spyfall game. ... Spyfall is designed by Alexandr Ushan and published by Hobby World. " 
    } ,
    {
        category : ["English","Drawing"],
        paid : "free" ,
        link : "https://skribbl.io/?8dBLMzsl4i1w" ,
        img : "./img/skribbl.jpg" ,
        name : "skribbl" , 
        description : "skribbl io is a free multiplayer drawing and guessing game. Draw and guess words with your friends and people all around the world!" 
    }
] ;